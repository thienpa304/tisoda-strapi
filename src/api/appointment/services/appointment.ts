...
export default factories.createCoreService('api::appointment.appointment', ({ strapi }) => ({
  async sendTelegramNotification(appointment: any) {
    ...
    const lines = [
      ...
      summary.place_name ? `• Địa điểm: ${sanitizeText(summary.place_name)}` : null,
    ...
    try {
      ...
      strapi.log.error('[appointment] Failed to send Telegram notification:', error);
    }
  },
}));
