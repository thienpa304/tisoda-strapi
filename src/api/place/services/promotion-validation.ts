...
export default {
   ...
   * @param promotion - Promotion rule to validate
   * @param placeServices - Array of place services for validation
   * @returns Array of validation errors (empty if valid)
   */
  validatePromotion(
    promotion: PromotionRule,
    placeServices?: Array<{ id: string | number; price?: number }>,
  ): ValidationError[] {
    ...
    if (promotion.discount_kind && promotion.amount !== undefined) {
      if (promotion.discount_kind === 'percent') {
        ...
        // Validate max amount <= max service price (if services provided)
        if (placeServices && placeServices.length > 0) {
          const maxServicePrice = Math.max(
            ...placeServices.map((s) => Number(s.price) || 0),
          )
          if (promotion.amount > maxServicePrice) {
            errors.push({
              field: 'amount',
              message: `Discount amount (${promotion.amount}) must not exceed the maximum service price (${maxServicePrice} VND)`,
            })
          }
        }
      }
    ...
    if (
      ...
      const startDate = promotion.start_at
