...
function validatePromotion(promotion: PromotionRule): ValidationError[] {
      ...
      if (startDate) {
        ...
        if (blackoutStart < startDate) {
          errors.push({
            ...
            message: 'Blackout start date must be within the promotion validity period',
          });
        }
      }
      ...
      if (blackout.end) {
        ...
        if (blackoutEnd < blackoutStart) {
          errors.push({
            ...
            message: 'Blackout end date must be greater than or equal to blackout start date',
          });
        }
        ...
        if (endDate) {
          ...
          if (blackoutEnd > endDate) {
            errors.push({
              field: `application.blackout_dates[${index}].end`,
              message: 'Blackout end date must be within the promotion validity period',
            });
          }
        }
  ...
  if (promotion.application?.booking_value_max !== undefined) {
    ...
    if (
      ...
      promotion.application.booking_value_max < promotion.application.booking_value_min
    ) {
      errors.push({
        field: 'application.booking_value_max',
        message: 'Maximum booking value must be greater than or equal to minimum booking value',
      });
    }
  }
